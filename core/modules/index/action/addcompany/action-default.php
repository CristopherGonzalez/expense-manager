...
if (!isset($_SESSION['user_id'])){
	...
	if (empty($_POST['name'])){
			...
			$company->password = sha1(md5(mysqli_real_escape_string($con,(strip_tags($_POST["password"],ENT_QUOTES)))));
			...
			if(isset($company_license) && !empty($company_license)){
				...
				$company->pais = intval($_POST['country']);
				$company->ciudad = intval($_POST['city']);
