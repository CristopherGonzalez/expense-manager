...
include "../core/modules/index/model/CategoryIncomeData.php";

// Propiedades del documento
$objPHPExcel->getProperties()->setCreator("Abisoft")
							 ->setLastModifiedBy("Abisoft")
							 ->setTitle("Office 2010 XLSX Documento de ingresos")
							 ->setSubject("Office 2010 XLSX Documento de ingresos")
							 ->setDescription("Documento de ingresos para Office 2010 XLSX, generado usando clases de PHP.")
							 ->setKeywords("office 2010 openxml php")
							 ->setCategory("Archivo con resultado de ingresos");
...
$objPHPExcel->setActiveSheetIndex(0)
			->setCellValue('A1', 'REPORTE DE INGRESOS')
...
$con=Database::getCon();
	...
	$query=IncomeData::getAll($_SESSION['company_id']);
	$cel=3;//Numero de fila donde empezara a crear  el reporte
	//while ($row=mysqli_fetch_array($query)){
	foreach($query as $inc){
			
			if($inc->category_id!=null){
				$category_id= $inc->getCategory()->name;
			}else{
			...
			// Agregar datos
			$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue($a, $inc->description)
            ->setCellValue($b, $inc->amount)
            ->setCellValue($c, $category_id)
            ->setCellValue($d, $inc->created_at);
		$cel+=1;
	}
